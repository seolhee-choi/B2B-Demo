...
const allMonths = Object.keys(monthlyCategorySales).sort()
const allCategories = [...new Set(rawData.map(d => d.product_category))]
...
const datasets = allCategories.map(category => {
  return {
    label: category,
    data: allMonths.map(month => monthlyCategorySales[month]?.[category] || 0),
